      ...
      setUsersOnTournametns((prev) => {
        ...
        return list.map((u) => {
          ...
          const prognoses = existing.some((p) => p.gameID === updatedPrognose.gameID)
            ? existing.map((p) => (p.gameID === updatedPrognose.gameID ? saved : p))
            : [...existing, saved];
          return {
            ...u,
            prognoses,
  ...
  const Raws = () => {
    ...
    return rows.map((user) => {
      ...
      games.forEach((game, gameIndex) => {
        const slot = user.prognoses?.find((p) => p.gameID === game.id);
        const prognose: Prognose = {
          gameID: game.id,
          game,
          exists: slot?.exists ?? false,
          visible: slot?.visible ?? false,
          id: slot?.visible ? slot.id : undefined,
          team1_result: slot?.visible ? slot.team1_result : undefined,
          team2_result: slot?.visible ? slot.team2_result : undefined,
          result: slot?.visible ? slot.result : undefined,
          cupResult: slot?.visible ? slot.cupResult : undefined,
          weightedResult: slot?.visible ? slot.weightedResult : undefined,
          autogenerated: slot?.visible ? slot.autogenerated : undefined,
          userOnTournamentUserID: slot?.userOnTournamentUserID ?? user.userID,
          userOnTournamentTournamentID:
            slot?.userOnTournamentTournamentID ?? currentTournament.id,
        };
        raw.push(
          <GameCell
            prognose={prognose}
            showCupResult={showCupResult}
