...
function GameCell(props: MyProps) {
  const { prognose, showCupResult = false, onPrognoseSaved, columnClassName = '' } = props;
  ...
  const color = isHidden
    ? ''
    : getPrognoseScoreCircleClass({
        result: shownPrognose.result,
        autogenerated: shownPrognose.autogenerated,
      });
  const displayScore = showCupResult
    ? shownPrognose.cupResult
    : (shownPrognose.weightedResult ?? shownPrognose.result);
  const scoreWide =
    typeof displayScore === 'number' && displayScore >= 10 ? ' score--wide' : '';
    ...
    >
      ...
      <div className="playerResultWrapper">
        ...
        {editable ? (
          ...
          <div className={`score ${color}${scoreWide}`}>
            {typeof displayScore === 'number' ? displayScore : prognose.exists ? "?" : "" }
          </div>
        )}
      </div>
    </td>
